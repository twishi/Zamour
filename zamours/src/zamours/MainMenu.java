...
public class MainMenu implements ApplicationListener {
	...
	public void create() {
		...
		boutonCliqueSprite = new Sprite(new Texture(
				Gdx.files.internal("boutonclique.png")));
		...
		xDecalage = xUnite(40); // pour gérer le décalage de positionnement
								// entre font et sprite
		yDecalage = yUnite(40);
	...
	public void manipulerMenu() {
		Gdx.input.setInputProcessor(new InputProcessor() {
			...
			public boolean touchUp(int x, int y, int pointer, int bouton) {
				...
				if (x > xUnite(180) && x < xUnite(300) && y > yUnite(115)
						&& y < yUnite(160)) {
				...
				if (x > xUnite(180) && x < xUnite(300) && y > yUnite(190)
						&& y < yUnite(235)) {
			...
			public boolean touchDown(int x, int y, int pointer, int bouton) {
				...
				if (x > xUnite(180) && x < xUnite(300) && y > yUnite(40)
						&& y < yUnite(88)) {
					cliqueBouton1 = true;
					soundTouchDown.play();
				}
	...
	public void dessinerMenu() // dessiner le menu
	{
		...
		batch.begin(); // obligatoire pour commencer un dessin sur le
						// SpriteBatch
		...
		font.draw(batch, "" + texteBouton1, xposBouton1 + xDecalage,
				yposBouton1 + yDecalage);

		// texte du bouton2

		font.draw(batch, "" + texteBouton2, xposBouton2 + xDecalage,
				yposBouton2 + yDecalage);

		// texte du bouton3

		font.draw(batch, "" + texteBouton3, xposBouton3 + xDecalage,
				yposBouton3 + yDecalage);
	...
	public void dessinerPage(int page) {
		...
		if (page == 1) // si on est à la page Game
			font.draw(batch, "The Game", xUnite(200), yUnite(320)); // dessiner
																	// le titre
																	// de la
																	// page 1
		if (page == 2) // si on est à la page Options
			font.draw(batch, "Options", xUnite(200), yUnite(320)); // dessiner
																	// le titre
																	// de la
																	// page 2
		if (page == 3) // si on est à la page Bonus
			font.draw(batch, "Bonus", xUnite(220), yUnite(320)); // dessiner le
																	// titre de
																	// la page 3
